...
The build is **stage by stage** (v0.1 shipped: foundation). The release sequence and each release's capability list are defined in `IMPLEMENTATION_GUIDE.md`. Each release gets its own contract file, `TEMPLATE_V0_N_SCOPE.md`, following the structure of the previous release's scope file.
...
2. The most recent scope file (highest-numbered `TEMPLATE_V0_N_SCOPE.md` in the repo root, currently `TEMPLATE_V0_1_SCOPE.md`) — this is your structural template, section by section.
