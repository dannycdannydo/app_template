...
readers and writers, the application access paths, raw SQL and
bulk/relationship access, and the connection types that reach the database.
...
| Metrics refresh | inside the API process (`app.main`) | `DATABASE_URL` | Application: aggregate counts only. **No DB GUC.** | Unchanged for unprotected ledgers; the protected `notification_deliveries` count uses the non-bypass `app_metrics` aggregate `app_attention_required_delivery_count()` (group 2, ADR-0022 decision 3) | Reads `outbox_events`, `jobs`, `job_attempts`, and the scalar attention-required delivery count. |
