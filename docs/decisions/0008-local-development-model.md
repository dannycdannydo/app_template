...
- `make dev` verifies both Redis services through host-facing
  `BROKER_REDIS_URL` and `RATE_LIMIT_REDIS_URL` before it starts native
  application processes. Compose health checks cannot detect a missing host
