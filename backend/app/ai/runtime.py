...
def _transfer_stores() -> dict[str, TransferStore]:
    ...
    v0.8 Scope §2.4/§6.4-§6.6: the ``storage_reference`` mode is served by the
    private same-region GCS staging adapter, and the ``provider_upload`` mode
    by the provider's own upload API (OpenAI Files API ``user_data`` upload
    with configured ``expires_after``; Anthropic beta Files API upload with
    the delete-only retention contract). Each store is built only when the
    deployment enabled the mode and the owning provider is enabled (the
    typed-settings validator already required the mode's configuration); a
    ...
    local_storage = _storage_is_local()
    ...
    if (
        TransferMode.PROVIDER_UPLOAD in deployment.enabled_transfer_modes
        and "anthropic" in settings.ai_enabled_providers
    ):
        stores["anthropic"] = AnthropicTransferStore(
            api_key=settings.ai_anthropic_api_key,
            base_url=settings.ai_anthropic_base_url,
            region=settings.ai_anthropic_inference_geography,
            timeout_seconds=settings.ai_http_timeout_seconds,
            # With a local storage seam the provider cannot fetch a signed URL
            # minted from it, so a transient source is served by the scratch
            # GCS staging directory plus a signed URL instead of a beta Files
            # API upload (Scope §6.6 lesson learned).
            stage_transient_locally=local_storage,
        )
    return stores
...
def _storage_is_local() -> bool:
    ...
    through GCS instead (v0.8 Scope §2.3, §6.4/§6.5). The predicate is shared
    with the typed deployment validator (``app/ai/deployment.py``) so the
    configuration closure and the wiring can never disagree.
    """
    settings = get_settings()
    presign_endpoint = settings.storage_public_endpoint_url or settings.storage_endpoint_url
    return storage_is_local(presign_endpoint)
...
def _managed_url_stager() -> ManagedUrlStager | None:
    ...
    deployment actually enables the managed-signed-url mode or the Anthropic
    local-transient path (``provider_upload`` with the anthropic provider
    enabled, Scope §6.6) — a local storage seam without either never pays for
    the wiring — and then the Vertex GCS configuration is required (fail fast
    at startup, BP §27). With a public HTTPS storage no stager is built and
    URLs are minted directly, so no copy is ever made.
    """
    if not _storage_is_local():
        return None
    policy = _transfer_deployment_policy()
    # The dev staging seam is needed for the managed-signed-url mode (a
    # retained source staged into GCS and served by a signed URL) and for the
    # Anthropic local-transient path (a transient source served by a scratch-
    # GCS signed URL document source instead of a beta Files API upload,
    # Scope §6.6 lesson learned).
    if TransferMode.MANAGED_SIGNED_URL not in policy.enabled_transfer_modes and not (
        TransferMode.PROVIDER_UPLOAD in policy.enabled_transfer_modes
        and "anthropic" in get_settings().ai_enabled_providers
    ):
        return None
    settings = get_settings()
