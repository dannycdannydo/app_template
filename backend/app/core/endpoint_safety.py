...
def validate_local_endpoint(endpoint: str) -> str:
    ...
    try:
        ...
        # Hostname over plain HTTP: only mDNS and single-label private-network
        # names are allowed; a dotted name is a public reachability risk.
        if host.endswith(_ALLOWED_HTTP_SUFFIXES) or "." not in host:
            return value
        raise ValueError(
    ...
    if address.is_loopback or address.is_private:
        return value
    raise ValueError(
        "ai_local_base_url over plain HTTP must be a loopback or private-range "
        "address; use https for any other host"
    )
