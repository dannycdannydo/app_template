...
The v0.8 storage-reference seam is exercised hermetically: the checked-in
registry bundle (``document.ask`` + the Vertex and fake models), a
:class:`FakeObjectStorage` source object, the deterministic
:class:`~app.ai.staging.FakeTransferStore` and an in-memory
:class:`TransferReferenceStore` stand in for the GCS staging path. These tests
prove the wiring itself — ``AIService.execute`` heads an oversized source,
streams it bounded into the staging seam, hands the adapter an opaque
``staged_file`` reference, records/cleans up the durable reference and never
touches the feature-owned source object — without Google credentials or
network access. The real GCS adapter and the durable SQL store are covered by
``test_ai_vertex_staging.py`` and ``test_ai_reference_db.py``.
"""
